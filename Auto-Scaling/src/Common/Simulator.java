...
public class Simulator{
	...
	private static double vmCost = 5.0;
	private static double slaViolationCost = 1.0;
...
//	public double run() throws IOException
	{		
		 int thrU = 92;
	        ...
	        int inU = 1;
	        int inL = 0;
		Simulator s = new Simulator(thrU, thrL, durU, durL, inU, inL);
...
//		monitor.SetInputFile("C:\\Users\\alinikravesh\\Dropbox\\MyPersonalFolder\\University\\Simulation\\UnpredictableWorkload_Experiment.xls");
		monitor.SetInputFile("C:\\Users\\alinikravesh\\Dropbox\\MyPersonalFolder\\University\\Simulation\\Evaluation\\periodic_test_predict.xls");
		monitor.SetInputFileActual("C:\\Users\\alinikravesh\\Dropbox\\MyPersonalFolder\\University\\Simulation\\Evaluation\\periodic_test_actual.xls");
...
//		ScalingUnitInterface decisionMaker = new AmazonDecisionMaker(60.0,100.0,app);
		...
		Timer timer = new Timer(monitoringInterval, monitor, s.decisionMaker, s.app);
		int time = 0;
		...
		s.app.EndExperiment(monitor.GetExperimentDuration()*monitoringInterval);
...
//		System.out.println("Excessive Operational Cost: "+ Integer.toString(timer.GetExcessiveOperationalCost()));
		int slacount = timer.GetSlaViolationCount();
		int vmcount = s.app.GetOperationalCost();
		double cost = (slacount * 1.0) + (vmcount * 1.0) ;
		
		System.out.println("Individual: <"+s.pthrU+", "+s.pthrL+", "+s.pdurU+", "+s.pdurL+", "+s.pinU+", "+s.pinL+">");
		System.out.println("Cost: "+ cost + "|| operational cost:"+ (s.app.GetOperationalCost() * 1) + ", SLA violation:" + (timer.GetSlaViolationCount() * 1));
		System.out.println("------------------");
//		return cost;
	}
}
